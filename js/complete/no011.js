...
function makePath(point, step, length) {
    var pairMap = function(i) {
        return [point[0] + i * step[0], point[1] + i * step[1]];
    };
    return operator.range(length).map(pairMap);
};
...
exports.main = function(verbose) {
    ...
    var data = fns.getData(11).split('\n').map(arrIntMap), x, y,
        vert = [], horiz = [], diagLR = [], diagRL = [];

    // UP/DOWN goes from data[x][y] to data[x+3][y] where 0 <= x, x+3, y <= 19
    for(x = 0; x <= 16; x++) {
        for(y = 0; y <= 19; y++) {
            vert.push(convert(makePath([x, y], [1, 0], 4), data));
        }
    }
    vert = Math.max.apply(Math, vert);

    // LEFT/RIGHT goes from data[x][y] to data[x][y+3] where 0 <= x, y, y+3 <= 19
    for(x = 0; x <= 19; x++) {
        for(y = 0; y <= 16; y++) {
            horiz.push(convert(makePath([x, y], [0, 1], 4), data));
        }
    }
    horiz = Math.max.apply(Math, horiz);

    // DIAGONAL L->R goes from data[x][y] to data[x+3][y+3] via +[1,1]
    for(x = 0; x <= 16; x++) {
        for(y = 0; y <= 16; y++) {
            diagLR.push(convert(makePath([x, y], [1, 1], 4), data));
        }
    }
    diagLR = Math.max.apply(Math, diagLR);

    // DIAGONAL R->L goes from data[x][y] to data[x-3][y+3] via +[-1,1]
    for(x = 3; x <= 19; x++) {
        for(y = 0; y <= 16; y++) {
            diagRL.push(convert(makePath([x, y], [-1, 1], 4), data));
        }
    }
    diagRL = Math.max.apply(Math, diagRL);
