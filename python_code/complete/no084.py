...
def next(landing_square, chance_card, chest_card):
    ...
    if landing_square == "G2J":
        ...
        if chance_card == 0:
            ...
            return next(back(landing_square, 3), chance_card, chest_card)
...
for place in xrange(GAME_PLAY):
    ...
    if doubles == 3:
        ...
        current, chance_card, chest_card = next(landing_square, chance_card, chest_card)
...
third, second, first = sorted(visited.values())[-3:]
top_visited = []
for key, value in visited.items():
    if value == first:
        index = SQUARES.index(key)
        top_visited.append(index)
...
if len(top_visited) != 3:
    raise Exception("Expected unique winners")

print ''.join([str(index).zfill(2) for index in top_visited])
